...
//////////////////////
	...
	obj_item = GameObject.Find("pig_cart_2p");
	gogo = 0;
	hoi = 0;
	posflag = 0;
...
//////////////////////	
	...
	Item1 = GameObject.Find("Barrier2");       
	Item2 = GameObject.Find("Shield2");         
	Item3 = GameObject.Find("Turtle2");          
	Item4 = GameObject.Find("UNAssigned2");
	Item5 = GameObject.Find("Booster2");
	Item6 = GameObject.Find("Recovery2");
...
function Controle () {
//////////////////////
	test = ctrler.GetComponent("Controller").direction2;
	connect = ctrler.GetComponent("Controller").P2_connect;
	
//	Debug.Log("test2 : " + test);
//	Debug.Log("gogo2 : " + gogo + " hoi2 " + hoi);
	...
	if(test.Length > 0)
	{
	  	...
	  	if(test[0] == 'I' || test[1] == 'I')
		{
		 // using item.
			if(b_Iflag == 0)
			{
				obj_item.GetComponent("getItem2").UseItem();
				b_Iflag = 1;
				Iflag = 0;
			}
		}
...
//		gogo = -0.3f;
	...
	if(test.Length <= 0 || test == "EE")
	{
		hoi = Input.GetAxis("Horizontal2");
	}
	if(currentSpeed < 0) hoi *= -1;
//////////////////////	
	currentSteerAngel *= hoi * dir1;
//////////////////////		
...
//	if(itemCnt0 == 1) GUI.Label(Rect(Screen.width/5 * 2, Screen.height/6 * 4, 400, 70), "Booster!!");
//	else if(itemCnt1 == 1) GUI.Label(Rect(Screen.width/5 * 2, Screen.height/6 * 4, 400, 70), "Confused!!");
//	else if(itemCnt2 == 1) GUI.Label(Rect(Screen.width/5 * 2, Screen.height/6 * 4, 400, 70), "UpsideDown!!");
//	else if(itemCnt3 == 1) GUI.Label(Rect(Screen.width/5 * 2, Screen.height/6 * 4, 400, 70), "Slow!!");
//	else if(itemCnt4 == 1) GUI.Label(Rect(Screen.width/5 * 2, Screen.height/6 * 4, 400, 70), "Shield!!");
//	else if(itemCnt5 == 1) GUI.Label(Rect(Screen.width/5 * 2, Screen.height/6 * 4, 400, 70), "Recovery!!");

	if(connect)
	{
		GUI.DrawTexture(Rect(Screen.width/2 - 100 , Screen.height/2 ,200,60),con);
	}
	else
	{
		GUI.DrawTexture(Rect(Screen.width/2 - 100 , Screen.height/2 ,200,60),dis);	
	}

	GUI.DrawTexture(Rect(Screen.width - 200,Screen.height/2 +20,80,80), inventory);
	GUI.DrawTexture(Rect(Screen.width - 100,Screen.height/2 +20,80,80), inventory2);
...
function Update(){
	...
	UsingItem();
//	Debug.Log("currentSpeed2 : " + maxTorque);
//	Debug.Log("cur2 " + currentSpeed + " maxTor2 " + maxTorque + " flag2 " + flag1);
...
function UsingItem()
{
	if(Time.time > itemTime + 1)
	{
		...
		{
			itemCnt1 = 2;
			if(shieldFlag == 0) dir1 *= -1;
		}
		if(itemCnt2 == 1)
		{
			itemCnt2 = 2;
			if(shieldFlag == 0) GameObject.Find ("Camera_2p").SendMessage("UpDown");
		}
		if(itemCnt3 == 1)
		{
			itemCnt3 = 2;
...
function UnBlind()
{
	GameObject.Find ("Camera_2p").SendMessage("UnUpDown");
	GameObject.Find ("Camera_2p").SendMessage("UnUpDown");
	itemCnt2 = 0;
}
