...
Car2 = GameObject.Find("pig_cart_2p");
}

function Update () {
	var hit : WheelHit;
	
	gogos = Car2.GetComponent(CarControlScript2).gogo;
	...
	if (skidAt <= CurrentFrictionValue && soundWait <= 0 || rpm < 300 && (Input.GetAxis("Vertical2") > 0 || gogos > 0) && soundWait <= 0 && rearWheel && hit.collider){
	Instantiate(skidSound, hit.point, Quaternion.identity);
	soundWait = 1;
	}
	soundWait -= Time.deltaTime*soundEmition;	
	
	if(Car2.GetComponent(CarControlScript2).itemCnt0 > 0)
	{
		skidSmoke.particleEmitter.emit = true;
		return;
	}
	
	if (skidAt <= CurrentFrictionValue || rpm < 300 && (Input.GetAxis("Vertical2") > 0 || gogos > 0) && rearWheel && hit.collider){
	skidSmoke.particleEmitter.emit = true;
	SkidMesh();
	}
	else {
